...
function create_config!(s::System, config::Vector{Endpoint}, perm::Vector{Int64}, worm_config::Vector{Endpoint})
    ...
    for n in eachindex(worm_config)
        ...
        # @info worm_config[n][1][3]
        # @info worm_config[n][2][3]
        ...
        if n==1 && length(worm_config)>1
            ps[length(config)+n] = Worm(r, V, zeros(s.M), missing, Int64(worm_config[n][2][3]), missing)
        elseif n==2
            ps[length(config)+n] = Worm(r, V, zeros(s.M), Int64(worm_config[n][1][3]), missing, length(config)+1)
        else
...
function test_remove_config(s::System)
    config = [[Coord([-1.0, -1.0]), Coord([-1., -1.])]]
    # config = Vector{Endpoint}()
    worm_config  = [[Coord([-1.1, -1.1, Int64(1)]), Coord([-0.9, -0.9, Int64(10)])]]
    # worm_config  = Vector{Endpoint}()
    perm = [1]
...
function main()
    ...
    s = System(potential; dim = 2, M = 100, N = 5, L = 4.0, T = 1, g=1.16, μ = 4.0, C = SectorCounter(1/((4.0)^2*100*50)), worms = true, propint = propint, rₐ = 1.0)

    IR = Threshold(50, 0, 0, 1, s.M, 0.4, 0.6)
    remove_accepted = 0
    for _ in 1:1
        test_remove_config(s)
